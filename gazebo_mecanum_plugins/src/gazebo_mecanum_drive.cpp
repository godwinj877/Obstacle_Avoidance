...
namespace gazebo_mecanum_plugins
{
  ...
  void GazeboMecanumRosDrive::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    // Dynamic properties
    impl_->max_wheel_accel_ = _sdf->Get<double>("wheelAccel", 5.0).first;
    impl_->max_wheel_torque_ = _sdf->Get<double>("WheelTorque", 50.0).first;
    ...
    impl_->cmd_topic_ = _sdf->Get<std::string>("commandTopic", "gazebo_cmd_vel").first;
  ...
  void GazeboMecanumRosDrivePrivate::calkinematics(linear_vel &line_vel)
  {
    ...
    double l = 2.0 / (wheel_w_separation_[0] - wheel_l_separation_[0]);

    line_vel.vel_x = (wheel_encoder[LEFT_REAR] + wheel_encoder[LEFT_FRONT] + wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) * wheel_diameter_[0] / 8;
    line_vel.vel_y = (-wheel_encoder[LEFT_REAR] + wheel_encoder[LEFT_FRONT] - wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) * wheel_diameter_[0] / 8;
    line_vel.vel_th = (-wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] + wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) * l * wheel_diameter_[0] / 8;
  }
