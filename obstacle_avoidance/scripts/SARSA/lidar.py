...
def scanDiscretization(lidar):
    ...
    # Find the left left side lidar values of the vehicle
    lidar_left_right = np.min(lidar[(ANGLE_MIN + HORIZON_WIDTH):(ANGLE_MIN + 2*HORIZON_WIDTH)])
...
def checkCrash(lidar, prev_action):
    ...
    if prev_action == 0:
        lidar_horizon =  np.concatenate((lidar[ANGLE_MIN:ANGLE_MIN + HORIZON_WIDTH] , lidar[ANGLE_MAX - HORIZON_WIDTH:ANGLE_MAX]), axis=0)
    elif prev_action == 1:
